...
echo "/u01/app/oracle/product/21.0.0/rdbms/runInstaller -ignorePrereq -waitforcompletion -silent -responseFile /tmp/setuprdbms.rsp" > /tmp/install_client_ora.sh
chmod +775 /tmp/install_client_ora.sh
sudo -H -u $RDBMS_OWNER bash -c '/tmp/install_client_ora.sh'
